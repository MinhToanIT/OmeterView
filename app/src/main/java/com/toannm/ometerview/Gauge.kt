...
import com.toannm.ometerview.components.Section
import com.toannm.ometerview.util.OnSectionChangeListener
import com.toannm.ometerview.util.OnSpeedChangeListener
import com.toannm.ometerview.util.doOnSections
import java.util.*
...
abstract class Gauge constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr), Observer {
    ...
    private fun getSpeedUnitTextWidth(): Float =
        if (unitUnderSpeedText)
            max(speedTextPaint.measureText(getSpeedText().toString().substring(0,getSpeedText().toString().indexOf("."))), unitTextPaint.measureText(unit))
        else
            speedTextPaint.measureText(getSpeedText().toString().substring(0,getSpeedText().toString().indexOf("."))) + unitTextPaint.measureText(unit) + unitSpeedInterval
    ...
    protected fun drawSpeedUnitText(canvas: Canvas) {
        ...
        updateSpeedUnitTextBitmap(getSpeedText().toString().substring(0,getSpeedText().toString().indexOf(".")))
